...
export default function IncidentHistoryPanel({ worker, alerts, incidents }: IncidentHistoryPanelProps) {
  return (
    <Paper className="history-panel activity-panel" elevation={0}>
      <div className="history-panel-heading"><div><h2>Incident & alert history</h2><p>Operational events associated with this team member</p></div><Button className="text-button">View report</Button></div>
      <div className="history-timeline">
        ...
        {alerts.length + incidents.length === 0 && <div className="history-empty"><span><Icon name="check" size={22} /></span><strong>No recorded events</strong><p>This employee has a clear activity history.</p></div>}
      </div>
    </Paper>
  );
}
